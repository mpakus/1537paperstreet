...
#[tauri::command(rename_all = "snake_case")]
pub(crate) async fn text_format(rel_path: PathBuf, text: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || format_text(&rel_path, &text))
        .await
        .map_err(|error| error.to_string())
        .and_then(|result| result.map_err(to_command_error))
}
...
pub(crate) async fn updates_check() -> Result<UpdateCheck, String> {
    tauri::async_runtime::spawn_blocking(|| crate::updates::check_latest(env!("CARGO_PKG_VERSION")))
        .await
        .map_err(|error| error.to_string())
